...
#ifndef _AAPREDEFINE_H
#define _AAPREDEFINE_H
...
#define SET_ACCESSOR(t, y) void Set##y(t x) { m_##y = x;}
#define GET_ACCESSOR(t, y) t Get##y() const { return m_##y;}
#define SET_GET_ACCESSOR(t, y) SET_ACCESSOR(t, y) GET_ACCESSOR(t, y)
#define GET_SET_ACCESSOR(t, y) SET_GET_ACCESSOR(t, y)

//the handler
typedef void (*MouseHandler)(void *user_data);
typedef void (*KeyboardHandler)(UINT VK, void *user_data);
...
HRESULT CGrowableArray<TYPE>::SetSize( int nNewMaxSize )
{
    ...
    if( nOldSize > nNewMaxSize )
    {
        ...
        for( int i = nNewMaxSize; i < nOldSize; ++i )
            m_pData[i].~TYPE();								//execute the dtor but not free the memory
    }
...
HRESULT CGrowableArray<TYPE>::SetAt( int nIndex, const TYPE& value )
{
    ...
    m_pData[nIndex] = value;	//type must implement a safe copy constructor
    return S_OK;
}
...
int CGrowableArray<TYPE>::IndexOf( const TYPE& value, int iStart, int nNumElements )
{
    ...
    {
        if( value == m_pData[i] )		//type must override == operator
            return i;
    }
